 ...
 * but also relied on unconditionally -- no #ifdef __linux__ guard at the
 * call site -- by Wayland core's own src/wayland-client.c, confirmed for
 * real on both Linux x64/WSL and Windows x64 the first time this project
 * compiled that file for either target): like poll(), but with a struct
 * timespec timeout and an optional sigmask atomically installed for the
 * duration of the wait.
 *
 * Deliberately portable (no CRT_TARGET_OS_LINUX guard, unlike an earlier
 * version of this function that called the Linux-only raw __crt_sys_ppoll
 * syscall trampoline directly): implemented in terms of the already
 * cross-platform poll() instead, exactly mirroring how pselect() above is
 * already implemented in terms of the already cross-platform select() --
 * same millisecond-timeout conversion shape, same sigprocmask()-based
 * "atomic enough" sigmask technique (including __crt_signal_delivery_
 * generation()'s own lost-wakeup check; see pselect()'s own comment for
 * the full reasoning, identical here). The only real behavioral cost
 * versus a true single-syscall ppoll(2) on Linux is millisecond instead
 * of nanosecond timeout precision -- irrelevant to every caller this
 * project has today, all of which just want "wait up to N milliseconds",
 * and avoids needing a second, Windows-side timeout-wait primitive
 * purely to give this one function real sub-millisecond precision on one
 * platform. */
int ppoll(struct pollfd* fds, nfds_t nfds, const struct timespec* tmo_p, const sigset_t* sigmask) {
  ...
  if (tmo_p != 0) {
    ...
    if (tmo_p->tv_sec > ((time_t)2147483)) {
      timeout_ms = 2147483647;
    } else {
      timeout_ms = (int)(tmo_p->tv_sec * 1000 + (tmo_p->tv_nsec + 999999) / 1000000);
    }
  }
