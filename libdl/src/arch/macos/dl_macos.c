...
/* Real dlsym() support for macOS.
 *
 * dyld's own image-introspection API (_dyld_image_count/_dyld_get_image_*)
 * is public and still works correctly on modern macOS, including for images
 * that live only inside the dyld shared cache (there are no true on-disk
 * system dylibs left as of macOS 11 "Big Sur"). What does NOT work against
 * the shared cache is the legacy NSAddImage()/NSLookupSymbolInImage() API
 * (deprecated since Mac OS X 10.5): its symbol lookup expects a classic
 * per-image nlist table, but shared-cache images store their exports as a
 * compact "export trie" instead, which that legacy API has no knowledge of.
 * See docs/dynamic_loading.md for the reproduction that proved this.
 *
 * This file parses the Mach-O export trie directly (LC_DYLD_EXPORTS_TRIE, or
 * the older LC_DYLD_INFO[_ONLY] export_off/export_size, for images built
 * before that load command existed) instead of relying on NSLookupSymbolInImage.
 * The trie-walk algorithm below mirrors dyld's own
 * MachOLoaded::trieWalk()/findExportedSymbol() (apple-oss-distributions/dyld,
 * dyld3/MachOLoaded.cpp), reimplemented in C. Only 64-bit Mach-O is handled,
 * matching this project's x86_64/aarch64-only scope.
 */
 ...
 * (unlike NSAddImage/NSLookupSymbolInImage below). */
 ...
 * image list above instead, without going through this API at all. */
extern const struct crt_mach_header_64* NSAddImage(const char* image_name, unsigned long options);
...
void* crt_dl_backend_open(const char* filename, int flags) {
  const struct crt_mach_header_64* image;

  (void)flags;
  if (filename == 0) {
    return CRT_DL_MAIN_HANDLE;
  }
  image = find_loaded_image_by_name(filename);
  if (image != 0) {
    return (void*)image;
  }
...
void* crt_dl_backend_sym(void* handle, const char* symbol) {
  ...
  if (handle == RTLD_DEFAULT || handle == CRT_DL_MAIN_HANDLE) {
    ...
    for (i = 0; i < count; ++i) {
      address = find_exported_symbol_in_image(_dyld_get_image_header(i), mach_symbol, 0);
      if (address != 0) {
        return address;
      }
    }
  } else {
    address = find_exported_symbol_in_image((const struct crt_mach_header_64*)handle, mach_symbol, 0);
    if (address != 0) {
      return address;
    }
  }
