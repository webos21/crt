...
- **Root-caused and fixed libpng's Windows shared-library build for real (`static-pass` -> `shared-pass`), instead of the documented-but-abandoned earlier attempt.** Per explicit direction to fix the actual root cause rather than patch around it (a generated-`libtool`-script post-processing shortcut was considered and rejected in favor of this). Four independent, genuine bugs found and fixed in order, each verified against the real `configure && make && make install` flow and the full `ctest` suite (79/79 throughout):
  ...
  - **Two more things found and fixed/documented along the way, both real but deliberately not chased further this session:**
    - `contrib/tools/png-fix-itxt`'s libtool-generated wrapper hits undeclared `_getcwd`/`_stat`/`_putenv`/`_spawnv`/`_P_WAIT` -- a different branch of the same generic `ltmain.sh` wrapper template than the `malloc.h` one above, apparently gated on a macro this recipe's CFLAGS don't currently touch. Left open: `png-fix-itxt.exe` simply fails to build for `make check` purposes; the library itself, its real install, and every other contrib tool build and link successfully either way.
...
- Root-cause `contrib/tools/png-fix-itxt`'s libtool-generated wrapper hitting
  undeclared `_getcwd`/`_stat`/`_putenv`/`_spawnv`/`_P_WAIT` (see the libpng
  shared-pass entry above) -- narrow, does not block libpng's own shared-pass
  status.
