...
{
	/* CRT: intptr_t, not long -- long is 32 bits on LLP64 (Windows),
	 * which truncated real bison-stack round trips through this pair
	 * of functions on that platform. Still just as much "swearing
	 * that p fits" as upstream's own comment: this is only ever used
	 * to smuggle a small int through a Node*-typed bison stack slot,
	 * never a real pointer dereferenced as one. */
