...
- **Real macOS/Metal hardware verification of Ganesh-to-surface
  ...
  everywhere" as real-hardware-verified on all three GPU-presentation
  hosts.** `skia_bridge.cc`'s own Metal branch had never actually been
  compiled anywhere before this session (it needs real Apple SDK headers
  via `xcrun`, unavailable in every environment that touched this code
  before) -- doing so for the first time, on real Apple Silicon hardware,
  immediately surfaced a real, previously-unreachable linkage bug: `gpu_
  internal.h` (shared by every per-backend `.c` file and by `skia_
  bridge.cc` itself, a `.cc` TU) declared `crtgfx_gpu_metal_surface_
  prepare_ganesh_present()` and its siblings with no `extern "C"` guard,
  unlike the already-guarded public `crtgfx/gpu.h`. The Vulkan/D3D12
  branches of `skia_bridge.cc` never hit this because they only read
  struct *fields* from this header, never call a *function* declared in
  it -- Metal's new hook is the first one this file actually calls, so a
  C++ compile mangled the call site's expected symbol while `gpu_
  metal.c`'s own C definition stayed unmangled: real link-time "symbol(s)
  not found," confirmed via the linker's own diagnostic (it matched the
  unmangled symbol already sitting in `libcrtgfx.a` and named the missing
  `extern "C"` directly). Fixed by wrapping this header's own function-
  declaration block in the same `#ifdef __cplusplus extern "C" { ...`
  guard `crtgfx/gpu.h` already established (`libcrtgfx/src/gpu_
  internal.h`) -- a one-file fix, no other source needed touching.

  Verification on macOS arm64: full `crtgfx-skia-smoke` preset rebuild
  clean; `ctest` 116/116 (`crtgfx_skia_gpu_offscreen_smoke_runs`
  included, zero regression). `crtgfx_skia_gpu_window_demo` run against a
  real, visible on-screen window; `screencapture` (working cleanly in
  this environment, unlike the Windows session's own failed `PrintWindow`/
  GDI capture attempts) gave genuine visual proof rather than only a
  zero-error log -- a screenshot taken mid-run, cropped and upscaled
  around the wrapped surface's own top-left corner, shows exactly the
  expected shared reference scene (`tests/skia_reference_scene.h`): a
  green top-left triangle, a red-to-blue top-right linear gradient, and
  an alpha-blended pink-toned bottom-left overlay, matching that file's
  own documented per-quadrant layout by eye. A separate bounded
  120-frame run exited 0 cleanly. Still open on macOS: x86_64 native
  execution (only arm64 exercised), pixel-exact (not just visually-by-eye)
  framebuffer verification, and live resize composed with this Ganesh-
  wrap path specifically (each verified real separately -- plain resize,
  Ganesh-wrap alone -- but this session's own coordinate-based window-drag
  automation, with no Accessibility-API grant available to query real
  window bounds, proved too imprecise to reliably re-drive a combined
  check). See `docs/libcrtgfx_wayland_plan.md`'s own "Ganesh-to-surface
  rendering" section and `TODO.md` for the full per-host trail.

## 2026-09-07

- **Wired the offscreen Ganesh pipeline onto a live GPU surface's own
  acquired image, closing out TODO.md's "Finish live GPU presentation
  everywhere" step in full.** New `crtgfx_skia_wrap_gpu_surface()`/
  `crtgfx_skia_gpu_surface_present()` (`libcrtgfx/include/crtgfx/skia.h`,
  `libcrtgfx/src/skia_bridge.cc`) wrap `crtgfx_gpu_surface_acquire()`'s
  own real swapchain/layer image as a real, GPU-backed `SkSurface` via
  `SkSurfaces::WrapBackendRenderTarget()` -- a render target, not a
  sampled texture, matching none of the three real swapchains/layers
  this project creates requesting sampling usage -- instead of letting
  Ganesh allocate its own backing image the way `crtgfx_skia_make_gpu_
  offscreen_surface()` already does. A new per-backend `ganesh_wrapped`
  guard field (`libcrtgfx/src/gpu_internal.h`) rejects mixing this path
  with the existing solid-color `crtgfx_gpu_surface_clear()`/`crtgfx_gpu_
  surface_present()` path mid-frame, the same out-of-order-misuse
  discipline every other guard in this contract already has.

  Real per-backend present mechanics, confirmed directly against this
  repo's own vendored Skia checkout (not memory), turned out to differ
  materially:
  - **Vulkan** (`gpu_vulkan.c`'s own fields): one `GrDirectContext::
    flush(surface, GrFlushInfo, &newState)` call does the entire real
    finalization -- Ganesh itself signals the *existing* `vk_render_
    finished_semaphore` (via `GrFlushInfo::fSignalSemaphores`, no new
    semaphore needed) and transitions the image to `VK_IMAGE_LAYOUT_
    PRESENT_SRC_KHR` (via `skgpu::MutableTextureStates::MakeVulkan()`),
    so the existing, unchanged `crtgfx_gpu_vulkan_surface_present()`
    presents afterward with zero changes of its own.
  - **D3D12** (`gpu_win32.c`'s own fields): no equivalent flush-time
    mechanism exists at all (confirmed: no `include/gpu/d3d/...
    MutableTextureState.h` in this vendored checkout) -- real, manual
    work instead: read back whatever resource state Ganesh's own flush
    left the wrapped back buffer in (`GrBackendRenderTargets::
    GetD3DTextureResourceInfo()`), record one small extra
    ...
    since `_clear()` was not called), sign the same fence `_clear()`
    already uses. Uses the *real* `<d3d12.h>` COM interfaces already
    force-included in `skia_bridge.cc` (`GrD3DTypes.h`) directly --
    `reinterpret_cast`-ing this surface's own `void*` fields straight to
    their real `ID3D12GraphicsCommandList`/`ID3D12CommandQueue`/
    `ID3D12Fence` types, no new `gpu_win32.c` hook needed.
  - **Metal** (`gpu_metal.c`'s own fields): simplest of the three --
    `MTLTexture` has no image-layout/resource-state concept at all, so
    the only real per-host work is a new, small `gpu_metal.c` hook
    (`crtgfx_gpu_metal_surface_prepare_ganesh_present()`) handing
    Ganesh's already-flushed drawable a fresh presenting command buffer
    from the same shared queue, mirroring `crtgfx_gpu_metal_surface_
    clear()`'s own last step -- kept in `gpu_metal.c` rather than `skia_
    bridge.cc` (unlike the other two) because that file already owns
    100% of this project's real Objective-C/Metal ABI knowledge and
    `skia_bridge.cc`'s own Metal branch hand-declares none of it.
  ...
  **A real bug found and fixed the same day, via live testing, not
  ...
  submit(GrSyncCpu::kNo)` on every backend. A real, live resize
  immediately after the first Ganesh-drawn frame on Windows/D3D12 failed
  `CRTGFX_ERROR_HOST` from `IDXGISwapChain3::ResizeBuffers()`. Root
  cause: Ganesh defers actually releasing a wrapped resource's own extra
  COM reference until it has confirmed the GPU is truly done with it, not
  merely once the caller's own `sk_sp<SkSurface>` is reset -- an async
  submit leaves that release still pending when the very next frame's
  `crtgfx_gpu_surface_resize()` call runs, and DXGI's own `ResizeBuffers()`
  requires every real reference to the previous images to be gone first.
  Fixed by using `GrSyncCpu::kYes` (a synchronous submit) on all three
  backends, not just the one caught failing -- the same real class of
  risk applies in principle to Vulkan/Metal too, even though only the
  Windows path could be live-tested this session. A genuinely missing
  `#include "include/core/SkColorSpace.h"` (needed for `sk_sp<SkColorSpace>`'s
  own destructor to see a complete type when passing `nullptr` to
  `WrapBackendRenderTarget()`'s `colorSpace` parameter) was also found and
  fixed the same way, via a real build failure, not anticipated up front.

  New manual demo, `crtgfx_skia_gpu_window_demo` (`libcrtgfx/tools/
  skia_gpu_window_demo.cc`, wired in the same dedicated `crtgfx-skia-
  smoke` build directory as `crtgfx_skia_gpu_offscreen_smoke`, never
  `add_test()`-registered -- needs a real desktop session): draws the
  exact same shared reference scene (`tests/skia_reference_scene.h`) the
  offscreen smoke test already proves pixel-correct, but through this new
  wrap/present pair onto a real on-screen window every frame instead, and
  reuses the resize demo's own `CRTGFX_EVENT_RESIZE`-draining loop
  unchanged.
  ...
  additions (zero regression). `crtgfx_skia_gpu_window_demo` ran against a
  real, visible window: a 60-frame and a 3000-frame run both completed
  cleanly (exit 0); a live-resize pass (the same `SetWindowPos`-script
  technique the earlier resize work used) is what caught the `GrSyncCpu`
  bug above -- after the fix, 7 live resizes across a 2000-frame run all
  succeeded and the run completed cleanly. Screenshot-based pixel
  verification was attempted but not achievable this session
  (`PrintWindow`/GDI `BitBlt`/`CopyFromScreen` all failed or returned a
  blank capture in this sandboxed session -- a real environment
  limitation on screen capture, not a rendering problem, distinguished
  from the demo's own zero-error frame-by-frame log). Full Windows
  `ctest` 133/133 (plain build) and the registered `crtgfx_skia_gpu_
  offscreen_smoke_runs` (skia-smoke build) both clean. **Linux**:
  `crtgfx_skia_gpu_offscreen_smoke` and the new `crtgfx_skia_gpu_window_
  demo` both build and link cleanly via WSL, and the offscreen smoke test
  passes in full there too (same real, hardware-backed `dzn` Vulkan
  device the earlier Linux GPU work already used); full plain Linux
  `ctest` 116/116 (`termios_echo_roundtrip_test` excluded, the known
  pre-existing non-interactive-WSL-runner limitation). **macOS**: `gpu_
  metal.c`'s own new hook and guard additions cross-compile cleanly to
  Mach-O objects on both `arm64`/`x86_64-apple-macos11` -- `skia_
  bridge.cc`'s own Metal branch could not be cross-compiled at all this
  session (it needs real Apple SDK headers via `xcrun --sdk macosx
  --show-sdk-path`, only available on real macOS/Xcode, a genuine
  structural gap distinct from every other macOS-only piece of this
  project, which deliberately avoids needing real SDK headers at all).
  Real on-screen macOS verification (does the reference scene actually
  render through a real Ganesh-wrapped `CAMetalLayer` drawable) remains
  the user's own separate step on real hardware. See `docs/
  libcrtgfx_wayland_plan.md`'s own "Ganesh-to-surface rendering" section
  for the same full trail.
...
- **Wayland core external build: `crtgfx-wayland-configure`/`-build`/**
  **`-smoke` CMake targets, verified end to end on Linux x64/WSL --**
  **including a real live-compositor wire-protocol round trip.** New
  ...
  the same way Skia's own recipe is pinned). Scope decided explicitly with
  the user before any code was written: wayland-scanner + wayland-client
  only for this pass (server/cursor/egl and wayland-protocols/xdg-shell
  deferred -- `crtgfx-wayland-smoke`, the only current consumer, only
  needs the client library and core `wayland.xml`); the smoke test is a
  wholly standalone executable (new `libcrtgfx/tests/wayland_client_
  smoke.c`), deliberately never touching the existing hand-rolled
  `libcrtgfx/src/arch/linux/window_wayland.c` backend.
  **The build mechanism itself changed mid-session, at the user's own
  explicit direction**: a first version used Meson/Ninja (still compiling
  through `tools/crt-cc`), which worked in principle but needed
  meson/ninja/pkg-config as real host build tools this project's own
  sandboxed tooling cannot install. After the user pointed this session at
  a manual embedded-cross-compile guide and asked to remove that
  dependency entirely, `tools/build_wayland.py` was rewritten to drive
  every compile/link step directly via `tools/crt-cc` itself -- no
  Meson/Ninja/pkg-config at all. Two real inaccuracies in the pasted guide
  were caught against the real, pinned Wayland source before implementing
  (wayland-scanner needs expat, not libxml2 -- confirmed via `src/
  scanner.c`'s own unconditional `#include <expat.h>` vs. libxml2's
  `#if HAVE_LIBXML`-gated one; `connection.c`, the wire-protocol
  marshaling file, was missing from the guide's own compile line
  entirely). Every generated-file step (the DTD-to-header embed, the
  `wayland-version.h` substitution, `wayland-scanner`'s own `-c client-
  header`/`public-code` CLI modes) replicates real upstream mechanics,
  confirmed by fetching and reading `src/embed.py`/`src/scanner.c`/`src/
  wayland-version.h.in`/`src/meson.build` directly from the pinned commit,
  not guessed. Static archive only (`libwayland-client.a`, no shared
  `.so` yet) -- avoids a real two-copies-of-this-project's-own-libc risk a
  shared build linked into an otherwise-statically-linked smoke executable
  would create, and has no current consumer to justify the extra SONAME-
  versioning work yet.
  `porting/recipes/expat.json` (new, previous entry) covers wayland-
  scanner's own expat dependency; the existing `libffi` port covers
  `connection.c`'s own closure-based argument marshaling. Both share one
  `port_prefix` per preset build directory, so `tools/build_wayland.py`
  takes a single `--port-prefix` flag and links both by explicit full
  path -- no `.pc`/pkg-config resolution exists any more since Meson (and
  its pkgconfig module) is gone.
  Three real, general CRT/PAL gaps were found and fixed compiling
  Wayland's own real sources against this project's libc for the first
  time, none routed around:
  1. **`SO_PEERCRED`/`struct ucred`/`MSG_CMSG_CLOEXEC`/`MSG_DONTWAIT`/
     `MSG_NOSIGNAL`** added to `include/sys/socket.h` (Linux-only, real
     UAPI values) -- `src/wayland-os.c`'s own `wl_os_socket_peercred()`
     has a hard `#elif defined(SO_PEERCRED)` / `#else #error "Don't know
     how to read ucred on this platform"` with no portable fallback (every
     *other* optional feature that file checks -- `HAVE_ACCEPT4`, `HAVE_
     SYS_UCRED_H`, `HAVE_BROKEN_MSG_CMSG_CLOEXEC`, `HAVE_GETTID` -- has a
     real, already-present fallback and was simply left undefined via a
     new, deliberately near-empty generated `config.h`, matching what a
     genuinely minimal host's own real feature detection would answer for
     most of them anyway); `connection.c`'s own read/write path needed the
     three `MSG_*` flags. Verified directly: a standalone `crt-cc` compile
     referencing all three succeeds on Linux x64/WSL.
  2. **`ppoll()`** added to `include/poll.h`/`libc/src/poll.c` --
     `src/wayland-client.c`'s own dispatch loop calls it directly.
     `__crt_sys_ppoll`'s own raw syscall trampoline (both aarch64/x86_64
     `.S` files) already existed, used internally by `poll()`'s own real
     implementation, but was never exposed publicly. Implemented the real
     public API on top of it, reusing `pselect()`'s own already-
     established, already-documented `sigprocmask()`+`__crt_signal_
     delivery_generation()` "atomic enough, no lost wakeup" technique for
     the (here, unexercised but still real) non-NULL-sigmask case, rather
     than widening the trampoline to the full 5-argument `ppoll(2)`
     syscall shape purely for a sigmask this project's own first real
     caller never actually passes.
  3. **`epoll_create()`** added to `include/sys/epoll.h`/`libc/src/
     epoll.c` -- `wl_os_epoll_create_cloexec()`'s own `epoll_create1`-
     unavailable fallback path (dead code on any real modern kernel, but
     still unconditionally compiled). Implemented in terms of the
     already-present `epoll_create1(0)` (kernel-documented as exactly
     equivalent for any valid `size`) rather than a second raw syscall
     trampoline.
  A fourth, general bug (not Wayland-specific, but found preparing
  `libffi`, a Wayland build dependency, on a real Ubuntu/WSL host for the
  first time this session): `tools/crt-port-build.py`'s own `AR`/`RANLIB`/
  `STRIP` defaults silently missed a real, present LLVM install --
  Debian/Ubuntu's `llvm-<N>` package installs every LLVM tool unlinked
  under `/usr/lib/llvm-<N>/bin/`, exposing only `clang`/`clang++`/... on
  PATH via `update-alternatives` -- so this project's own fallback used
  the *host's* real GNU binutils `ranlib` instead, which then genuinely
  **segfaulted** installing `libffi.a` (`ranlib: Relink '.../sysroot/lib/
  libm.so' with '/usr/lib/x86_64-linux-gnu/libm.so.6' for IFUNC symbol
  'ceil'` immediately followed by `Segmentation fault`) -- a real GNU-
  ranlib IFUNC-relink heuristic choking on this project's own, non-host,
  sysroot `libm.so`. Fixed generally with a new `find_llvm_tool()` (used
  for AR/RANLIB/STRIP alike): resolve `clang`'s own `os.path.realpath()`
  and look for the requested tool in that same directory.
  A fifth, general bug in the CMake porting-recipe machinery itself,
  never hit before because nothing had previously combined a *nested*
  shadow build directory with a real `port-build-*`/`port-test-*` target:
  `CRT_BUILD_PRESET_NAME` (top-level `CMakeLists.txt`) fed `tools/
  crt-port-build.py`'s own `--preset` flag as just `get_filename_component
  (... NAME)` -- the basename only, correct for every real `CMakePresets.
  json` entry (`out/<preset-name>`, always one path component deep) but
  silently wrong for `tools/test_crtgfx_wayland_smoke.py`'s own
  `${CMAKE_BINARY_DIR}/crtgfx-wayland-smoke` shadow directory, two levels
  deep under `out/`: `crt-port-build.py`'s own `build_dir = root/"out"/
  preset` reconstruction silently dropped the `linux-host-ninja-debug/`
  prefix and looked for already-fetched port sources in the wrong
  directory entirely. Fixed generally: compute the real relative path
  from `${CMAKE_SOURCE_DIR}/out` to `CMAKE_BINARY_DIR` instead of just the
  basename, falling back to the old basename-only behavior if
  `CMAKE_BINARY_DIR` isn't under `out/` at all.
  **Verified for real, not just "it built"**: `crtgfx-wayland-smoke`
  fetched Wayland, built `wayland-scanner`, generated the client protocol
  bindings, built and installed `libwayland-client.a`, then compiled and
  ran `wayland_client_smoke` -- which found a real, live Wayland
  compositor already reachable in this session's own WSL environment
  (WSLg's own Weston/RDP-based one, confirmed by the real global names it
  enumerated: `weston_rdprail_shell`, `weston_screenshooter`, plus
  `wl_compositor`/`wl_shm`/`xdg_wm_base`/`wl_seat`/... 20 real globals
  total) and completed a genuine `wl_registry` wire-protocol round trip
  against it -- not merely the documented "no compositor, build/link
  verified" headless fallback this test was designed to also accept.
  Full local `ctest`: 104/104 on this same WSL/Linux x64 host after a
  full rebuild (a from-scratch `cmake --build` was needed to actually
  exercise the poll.h/socket.h/epoll.h changes -- an earlier `ctest`
  invocation in this same investigation had silently re-run stale, already
  -built binaries and does not count as a real regression check).
  Separately, verifying the equivalent Windows preset directory
  (`windows-host-ninja-debug`) surfaced -- and this session then re-fixed
  -- a real instance of the exact "transient bootstrap-cxx-overwrites-
  real-libc++" ordering artifact this file's own 2026-08-18 macOS entry
  already documents: this session's own many earlier, unrelated targeted
  `cmake --build ... --target sysroot`/`rootfs` calls (while iterating on
  the expat/Wayland CMake wiring) had re-triggered the top-level `sysroot`
  target's own bootstrap-`cxx`-reinstall, silently overwriting the real
  imported `libc++.a` `crt-libcxx-sysroot` had staged there earlier --
  `crtgfx_skia_raster_smoke`'s own link failed with the identical class of
  `std::__1::...` undefined-symbol errors that entry describes. Fixed the
  same documented way: re-ran `crt-libcxx-sysroot` to restage the real
  archive, then a full default `cmake --build`, then confirmed **100%
  tests passed, 121/121** on Windows too -- this was not a Wayland-caused
  regression, but a real, pre-existing, self-inflicted artifact from this
  same session's own earlier build activity in that directory, only
  caught now because this was the first *full* rebuild attempted there
  since.
  See `libcrtgfx/third_party/wayland/recipe.json`'s own notes, `tools/
  build_wayland.py`'s own docstring, and each libc change's own comment
  for the full trail.
