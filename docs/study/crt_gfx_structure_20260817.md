# Graphics Structure Study (2026-08-17)

> **문서 상태:** 상위 runtime 디렉터리 구조를 정하기 전 작성된 연구
> 기록이다. 제안된 `libcrtgfx`/`libcrtjs`/`libcrtmedia` 형상은 현재
> source tree에 반영되었으며, 현재 상태는 `docs/runtime_roadmap.md`와
> `STATUS.md`를 따른다.
