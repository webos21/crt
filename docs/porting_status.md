...
| libpng | 1.6.57 | `porting/recipes/libpng.json` | pending | configure-pass | configure-blocked | configure | Depends on zlib in `PORT_PREFIX`. Windows aarch64 `configure` (autoconf-generated, unlike zlib's hand-written one) exposed four real CRT/mksh bugs, all fixed and verified: no `regcomp()`/`regexec()` capture-group support, mksh not recognizing Windows drive-letter absolute paths, generic `pipe()` not routed through the spawn broker inside an unregistered clone, and `ERROR_INVALID_NAME` mismapped to `EIO` instead of `ENOENT`. `configure` still fails (exit 127, not yet root-caused) partway through automake's ustar-tar-format probe -- see `TODO.md`, "in progressing", for the full trail and where to resume. |
